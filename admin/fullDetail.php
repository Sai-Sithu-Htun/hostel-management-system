	...
	<div class="ts-main-content col-md-12">
			<?php include('includes/sidebar.php');?>
					...
					<div class="col-md-12" style="margin-top: 80px;">
						<h2 class="page-title">Full Student Details</h2>
						<div class="panel panel-default">
							...
							<div class="panel-body">
								<table id="" class="table table-bordered " cellspacing="0" width="100%">
									...
									<tbody>
<?php	
$aid = isset($_GET['stdid']) ? intval($_GET['stdid']) : 0;
$adminHostel = isset($_SESSION['hostelID']) ? intval($_SESSION['hostelID']) : 0;

// Validate the requested student ID and that the student belongs to the
// currently selected hostel before showing any personal details.
$studentBelongsToHostel = false;
if ($aid > 0 && $adminHostel > 0) {
	$stmtCheck = $mysqli->prepare("SELECT stayHostel FROM student WHERE studentID = ?");
	$stmtCheck->bind_param('i', $aid);
	$stmtCheck->execute();
	$stmtCheck->bind_result($studentHostel);
	if ($stmtCheck->fetch() && intval($studentHostel) === $adminHostel) {
		$studentBelongsToHostel = true;
	}
	$stmtCheck->close();
}

if (!$studentBelongsToHostel) {
	echo "<tr><td colspan=\"6\" class=\"info\">Access denied: student not found in the selected hostel.</td></tr>";
} else {

// Select from rooms table - match either slot (stdID or stdIdTwo) within this hostel
$ret_rooms = "SELECT * FROM rooms WHERE (stdID = ? OR stdIdTwo = ?) AND hostelID = ?";
$stmt_rooms = $mysqli->prepare($ret_rooms);
$stmt_rooms->bind_param('iii', $aid, $aid, $adminHostel);
$stmt_rooms->execute();
...
$stmt_studentinfo->bind_param('i', $aid);
